...
class Pems08_Dataset(Dataset):
    def __init__(self, eval_length=24, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
        ...
        meanstd_path = "./data/pems08/pems08_meanstd.pk"
        ...
        if missing_pattern == 'block':
            ...
            eval_mask = sample_mask(shape=data.shape, p=0., p_noise=0.25, max_seq=12, min_seq= 12 * 4, rng=self.rng)
        gt_mask = (1 - (eval_mask | (1 - ob_mask))).astype('uint8')

        val_start = int((1 - val_len - test_len) * len(data))
        test_start = int((1 - test_len) * len(data))
        c_data = ((np.nan_to_num(data) - self.train_mean) / self.train_std) * ob_mask
